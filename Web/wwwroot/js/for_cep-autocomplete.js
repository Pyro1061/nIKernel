...
document.addEventListener('DOMContentLoaded', function() {
    const cepInput = document.querySelector('input[name="Fornecedor.EnderecoCep"]');
...
function preencherCampos(dados) {
    ...
    const logradouroInput = document.querySelector('input[name="Fornecedor.EnderecoLogradouro"]');
    if (logradouroInput) {
        logradouroInput.value = dados.logradouro || '';
    }
    
    // Preenche o campo de bairro
    const bairroInput = document.querySelector('input[name="Fornecedor.EnderecoBairro"]');
    if (bairroInput) {
        bairroInput.value = dados.bairro || '';
    }
    
    // Preenche o campo de cidade
    const cidadeInput = document.querySelector('input[name="Fornecedor.EnderecoCidade"]');
    if (cidadeInput) {
        cidadeInput.value = dados.localidade || '';
    }
    
    // Preenche o campo de estado (UF)
    const estadoInput = document.querySelector('input[name="Fornecedor.EnderecoEstado"]');
    if (estadoInput) {
        estadoInput.value = dados.uf || '';
    }
    
    // Foca no campo de número para o usuário completar o endereço
    const numeroInput = document.querySelector('input[name="Fornecedor.EnderecoNumero"]');
    if (numeroInput) {
        numeroInput.focus();
    }
}
